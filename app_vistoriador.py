# -*- coding: utf-8 -*-
# ------------------------------------------------------------
# Painel de Produção por Vistoriador (Streamlit) - LOG VISTORIAS
# • Sem UI inicial (conexão silenciosa via planilha-índice)
...
# CONFIG BÁSICA
# =========================
st.set_page_config(page_title="🧰 Produção por Vistoriador - LOG VISTORIAS", layout="wide")
st.title("🧰 Painel de Produção por Vistoriador - LOG VISTORIAS")

# === Planilha-Índice (ARQUIVOS) ===
# Se existir secrets["log_index_sheet_id"], usa ele; senão, usa o ID abaixo.
INDEX_SHEET_ID = (st.secrets.get("log_index_sheet_id", "") or "1efKWTSRNafMRR4pLlRSgHTpn0-U64Tn-xSpL3_eL9vg").strip()
...
# =========================
# KPIs (agora 100% dependem de 'view')
# =========================
...
# metas do mês de referência (mais recente no recorte)
ref_ym = None
if not view.empty:
...
# normalização + FILTRO de tipo SÓ para esta tabela
...
sel_tipos = st.multiselect("Tipo (filtro apenas desta tabela)", options=tipo_options, default=tipo_options, key="resumo_tipo_filter_log")
...
if len(datas_ok) == 0:
    ...
    def render_ranking(df_sub, titulo):
        ...
        top_fmt = pd.DataFrame({
            ...
            "% Ating. (geral/meta)": top["ATING_%"].map(lambda p: f"{p:.0f}%"),
        })
        ...
        bot_fmt = pd.DataFrame({
            ...
            "% Ating. (geral/meta)": bot["ATING_%"].map(lambda p: f"{p:.0f}%"),
        })
...
if not dates_avail:
    ...
    metas_join = df_metas_all[df_metas_all["__YM__"] == ym_day][["VISTORIADOR","TIPO","META_MENSAL","DIAS_UTEIS"]].copy() if not df_metas_all.empty else pd.DataFrame(columns=["VISTORIADOR","TIPO","META_MENSAL","DIAS_UTEIS"])
